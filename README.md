...
`examples/故都的秋/` 是一次真实跑通的完整记录，可用来对照产出形态：

| 内容 | 文件 |
|---|---|
| 教学设计（十二节：依据／目标／课时表／任务链／主问题／板书／分层作业／预设应对／检测量规／分层参数） | `examples/故都的秋/故都的秋_教学设计与检测量规.md` |
| 课件成品 · 15 页 · 投给学生用 | `examples/故都的秋/课件/故都的秋.pptx` |
| 课件源工程（叙事稿／设计稿／逐页 DSL／配图） | `examples/故都的秋/课件/` |

**输入参数**：单篇精读 ｜ 2 课时 ｜ 普通校为主版本 ｜ 课标按 2025 修订版三级水平。
**读法**：先看 README 的六步产出摘要，再对照教学设计看「分析点是否落到语句」「教学点是否排除掉学生已会的内容」，最后打开课件看版式与字号是否守住了课件版红线。

> 注意：课件是**课件版**（投给学生），按红线**不含**教学目标、设计意图、预设应对、作业答案——这些在教学设计 md 里。要上屏说课需另出教案版。
>
> **示例早于"四份 Word"规范**：这份记录跑于 2026-09-13，当时只有教学设计 md 与课件 pptx，没有 `.docx`。它仍是校验产出形态的好样本（分析点是否落到语句、教学点是否排除掉学生已会的内容、课件是否守住课件版红线），但**不能作为四份 Word 的形态参照**——后者见 `references/deliverables-word.md`。
